...
# configure logger
logname = LOGS / "polynomial_regression.txt"
...
f = open(MODELS / "airport_encodings.json", "w")
...
plt.savefig(IMAGES / "model_performance_test.jpg", dpi=300)
...
with mlflow.start_run(experiment_id = experiment.experiment_id, run_name = "Final Model - Test Data"):
    ...
    mlflow.log_artifact(str(LOGS / "import_format_log.txt"))
    mlflow.log_artifact(str(LOGS / "clean_data_log.txt"))
    mlflow.log_artifact(str(IMAGES / "model_performance_test.jpg"))
    mlflow.log_artifact(str(LOGS / "polynomial_regression.txt"))
